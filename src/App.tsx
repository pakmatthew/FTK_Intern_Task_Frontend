...
const App = () => {
	...
	const fetchTaskData = async () => {
		try {
			...
			if (result.data.length > 0) {
				...
				for (
					let index = 0;
					index < tempCompletedTaskSeqNumber.length;
					index++
				) {
					if (index !== tempCompletedTaskSeqNumber.length) {
						if (
							tempCompletedTaskSeqNumber[index] >
							tempCompletedTaskSeqNumber[index + 1]
						) {
							let tempNumber;
	...
	const CreateNewTask = async () => {
		try {
			...
			if (newTaskName === "") alert("Error: No name input.");
			else if (newSeqNumber === "") alert("Error: No Sequence Number input.");
			else
				alert(
					`ERROR: Task cannot be created, reason: ${JSON.stringify(error)}`
				);
  ...
  const DeleteAllTask = async () => {
    try {
			...
			alert(`ERROR: All tasks cannot be deleted, reason: ${JSON.stringify(error)}`);
			console.log("error = ", error);
    }
  }

	const handleTaskClickAction = (data: Task, action: TaskClickAction) => {
		...
		if (action === TaskClickAction.EDIT) setIsEdit(true);
			...
			setNewTaskStatus(
				action === TaskClickAction.NEW ? TaskStatus.NEW : TaskStatus.COMPLETE
			);
		}
	};

	useEffect(() => {
		if (assignedTaskId !== "" && isEdit !== true && isDelete !== true) {
			EditTask();
		}
				...
				<div className={style.NewTaskContainer}>
					<div className={style.NewTaskContainerHeader}>
						<FStyledText
							...
							children="Plan to do:"
						/>
            <div className={style.NewTaskContainerHeaderButtonDiv}>
              <FButton
                type="primary"
              ...
              />
				...
				{isDeleteAll && (
					<PopUp title="Delete ALL tasks?" renderOpenPopup={setIsDeleteAll}>
						<FBottomNavigation
							leadingButtonName="Cancel"
							actionButtonName="Confirm"
							actionButtonColor={ColorTypes.RED}
							onLeadingButtonClick={() => setIsDeleteAll(false)}
							onActionButtonClick={DeleteAllTask}
						/>
					</PopUp>
				)}
			</div>
